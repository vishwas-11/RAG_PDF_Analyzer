...
const Dashboard = () => {
    ...
    try {
      ...
      if (res.data.files.length > 0) {
        setSelectedFile(prev =>
          forceSelect || !prev ? res.data.files[0] : prev
        );
      }
  ...
  // 🔹 Chats for selected file
  const activeChats = chatHistory.filter(
    chat => chat.fileName === selectedFile
  );
  ...
  return (
    <div className="flex h-screen">
      ...
      <aside className="w-72 border-r border-white/5 bg-[#0f051a]/60 backdrop-blur-3xl p-6 flex flex-col">
        <h2 className="text-xs font-black text-violet-500 uppercase tracking-widest mb-6">
          Documents
        </h2>
      ...
      <main className="flex-1 flex flex-col items-center overflow-y-auto p-8">
        <div className="w-full max-w-3xl space-y-10">
          ...
          {selectedFile ? (
            <ChatBox
              key={selectedFile}   //  IMPORTANT: re-render when file changes
